...
export function getFairPricingInsight(order) {
  ...
  if (!normalizedCrop) {
    return {
      mandiAvg: order.agreedPrice || 0,
      mandiMarket: 'Regional benchmark',
      grade: (order.grade || 'B').toUpperCase(),
      qualityFactor: 1,
      transportPerQ: 0,
      handlingPerQ: 0,
      recommended: order.agreedPrice || 0,
      fairRange: { min: order.agreedPrice || 0, max: order.agreedPrice || 0 },
      differencePct: 0,
      dealScore: 75,
      verdict: 'Insufficient crop data',
    };
  }
